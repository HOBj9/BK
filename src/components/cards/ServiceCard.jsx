  ...
  return (
    <motion.div variants={staggerItem} className="h-full">
      <Card className="flex h-full flex-col" padding="lg">
        ...
        <h3 className="text-xl font-bold tracking-tight text-slate-900 dark:text-slate-100 sm:text-[1.35rem]">
          {service.title}
        </h3>
        <p className="mt-3 flex-1 leading-[1.65] text-slate-600 dark:text-slate-300">
          {service.shortDescription}
        </p>
        <Link
          to={service.pagePath ?? ROUTES.SERVICE(service.slug)}
          className="mt-7 inline-flex items-center gap-2 rounded-lg font-semibold text-brand-700 transition-all duration-300 hover:gap-3 hover:text-brand-900 dark:text-brand-300 dark:hover:text-brand-200"
        >
          En savoir plus
