...
import type { ContentSection, SiteInfo } from '@/types';

type ContactKey = 'phone' | 'email' | 'address' | 'hours';
...
type ContactPageBodyProps = {
  infoSections?: ContentSection[];
};

export default function ContactPageBody({ infoSections }: ContactPageBodyProps) {
  ...
  const getSiteValue = (key: ContactKey): string => site[key as keyof SiteInfo] as string;
        ...
        <aside className="space-y-6 lg:col-span-2">
          {contactItems.map(({ icon, label, key, href: isLink }, index) => (
            <ContactInfoCard
              ...
              icon={icon}
              label={label}
              value={getSiteValue(key)}
              href={isLink ? (key === 'phone' ? phoneHref : `mailto:${getSiteValue(key)}`) : null}
              isLink={!!isLink}
              index={index}
            />
          ))}
