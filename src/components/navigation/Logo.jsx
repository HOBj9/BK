...
export default function Logo({ className = '' }) {
  return (
    <Link
      to={ROUTES.HOME}
      className={`group flex items-center gap-3 font-bold text-brand-900 transition-opacity hover:opacity-90 ${className}`}
    ...
    >
      ...
      <span className="hidden text-lg tracking-tight sm:block">
        BK <span className="text-brand-600">Débouchage</span>
      </span>
    </Link>
  );
}
